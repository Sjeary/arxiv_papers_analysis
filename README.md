...
脚本使用 OpenAI-compatible Chat Completions 接口。推荐使用环境变量配置，不要把 API Key 写进代码。
...
- `LLM_API_KEY`：API Key。
- `LLM_API_URL`：接口地址，例如 `https://api.deepseek.com/v1`。
...
```powershell
python .\code\arxiv_paper_research.py --interest "基于3DGS的自动驾驶occupancy任务，重点关注3D Gaussian Splatting/Gaussian scene representation用于occupancy prediction、occupancy forecasting、occupancy scene completion、semantic occupancy、BEV occupancy、voxel occupancy；同时覆盖自动驾驶occupancy任务中不使用3DGS但代表性强的方法。优先camera-only、多相机环视、自动驾驶场景；LiDAR-only、纯SLAM、纯navigation、survey综述降低优先级。" --candidate-results 300 --max-results 30 --categories cs.CV,cs.RO,cs.AI --refine-rounds 1 --screen-batch-size 30
```
...
```powershell
python .\code\arxiv_paper_research.py --interest "3D occupancy autonomous driving" --candidate-results 100 --max-results 30 --categories cs.CV,cs.RO --no-llm
```

限制论文提交日期：

```powershell
python .\code\arxiv_paper_research.py --interest "camera-only 3D occupancy prediction autonomous driving" --candidate-results 200 --max-results 30 --categories cs.CV,cs.RO --from-date 2024-01-01
```

只保留高相关结果：

```powershell
python .\code\arxiv_paper_research.py --interest "Gaussian Splatting occupancy autonomous driving" --candidate-results 300 --max-results 30 --categories cs.CV,cs.RO --min-score 60
```
...
- 任务名：例如 `occupancy prediction`、`occupancy forecasting`、`occupancy scene completion`。
- 表示方式：例如 `3D Gaussian Splatting`、`3DGS`、`Gaussian scene representation`、`BEV occupancy`、`voxel occupancy`。
- 应用场景：例如 `autonomous driving`、`camera-only`、`multi-camera`、`surround-view`。
- 降权内容：例如 `LiDAR-only`、`survey`、`navigation`、`pure SLAM`。

示例：

```text
我想找自动驾驶3D occupancy相关的最相关工作，重点关注基于3D Gaussian Splatting/3DGS/Gaussian scene representation的occupancy prediction、occupancy forecasting、occupancy scene completion、semantic occupancy、BEV occupancy、voxel occupancy。也请覆盖不使用3DGS但在自动驾驶occupancy任务中代表性强的方法。优先考虑自动驾驶场景、多相机环视、camera-only或camera-centric方法；如果是LiDAR-only、纯SLAM、纯navigation、survey综述，请降低相关性。
```
...
- 大候选池会增加 LLM 调用次数和费用。
- `--refine-rounds 1` 通常比较合适；更多轮数会显著增加运行时间。
